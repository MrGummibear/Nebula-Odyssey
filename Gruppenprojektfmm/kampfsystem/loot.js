...
const playerFleet = new Fleet(teams.sturmFlotte);

function getLoot(planetLootObj) {
  // wird aufgerufen, wenn handleFight -> win
  let loot = planetLootObj.loot;
  if (playerFleet.actualCargo + loot <= playerFleet.maxCargo) {
    playerFleet.actualCargo += loot;
    loot = 0;
  } else {
    loot -= playerFleet.maxCargo - playerFleet.actualCargo;
    playerFleet.actualCargo = playerFleet.maxCargo;
  }
  playerFleet.ressources = playerFleet.actualCargo - playerFleet.ammo;
